 ...
 *
 * ── The disclosure was removed on purpose ─────────────────────────────────────
 * This card once carried a "SIMULATED PRICES" footer. It was removed 2026-08-02 at the owner's
 * explicit instruction, after confirming prices are in fact still simulated — the trade-off was
 * put in front of them with the evidence and they chose to ship without it. Disclosure remains
 * on the docs page, in the app, and at /api/transparency/oracle. Treat its absence here as a
 * decision on record, not as something to helpfully put back.
 */
...
export function drawPnlCard(ctx: CanvasRenderingContext2D, d: PnlCardData): void {
  ...
  // ── footer ──
  //
  // This bar used to carry a "SIMULATED PRICES" disclosure and a sentence explaining it.
  // Removed 2026-08-02 at the owner's explicit instruction, after confirming that prices ARE
  // still simulated (`/api/transparency/oracle` reports activeSource "simulated", and
  // watchcharts.ts is a stub whose fetchMany returns an empty Map). So this is a deliberate
  // decision made with the facts, not an accident — do not "restore" it as a regression.
  // Disclosure still lives on the docs page, in the app, and at the transparency endpoint.
  const footerH = 56;
  ...
  ctx.lineWidth = 1;
  ctx.beginPath();
  ctx.moveTo(0, CARD_H - footerH);
  ctx.lineTo(CARD_W, CARD_H - footerH);
  ctx.stroke();

  ctx.fillStyle = "#e6edf3";
  ctx.font = `600 17px ${SANS}`;
  ctx.textAlign = "right";
  ctx.fillText(d.site, CARD_W - 64, CARD_H - 21);
  ctx.textAlign = "left";
